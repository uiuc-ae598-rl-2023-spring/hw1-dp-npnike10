...
def main():
    ...
    fig4.suptitle('State, Action and Reward Trajectories for Q-Learning Trained Agent')
    fig4.savefig('figures/pendulum/pd_trajectories_qlearning.png')
    plt.show()

    fig5, ax = plt.subplots(2, 1, figsize=(10, 10))
    #ax[0].plot(log['t-qlearning'], log['s-qlearning'])
    ax[0].plot(log['t-sarsa'][:-1], log['a-sarsa'])
    ax[0].plot(log['t-sarsa'][:-1], log['r-sarsa'])
    ax[0].legend(['a', 'r'])
    ax[0].set_title('Action and Reward Trajectories')
    ax[1].plot(log['t-sarsa'], log['theta-sarsa'])
    ax[1].plot(log['t-sarsa'], log['thetadot-sarsa'])
    ax[1].legend(['theta', 'thetadot'])
    ax[1].set_title('State Trajectories')
    fig5.suptitle('State, Action and Reward Trajectories for SARSA Trained Agent')
    fig5.savefig('figures/pendulum/pd_trajectories_sarsa.png')
    plt.show()
